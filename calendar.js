...
var J1900 = 2415020.5;                // Epoch (day 1) of Excel 1900 date system (PC) //!Mil001 this JD value is for day 2 of Windows
var J1900MS = 2415019.5;				//!Mil001 Epoch (day 1) of Excel 1900 date system (PC), i.e; 1899-12-31T00 due to a conception bug in Windows
var J1904 = 2416480.5;                // Epoch (day 0) of Excel 1904 date system (Mac)

//!Mil003 var NormLeap transferred to locale-##.js
...
function jd_to_gregorian(jd) {
    var wjd, depoch, quadricent, dqc, cent, dcent, quad, dquad,
		...
		month, day;		//!Mil003  these two variables added
...
var JULIAN_EPOCH = 1721423.5;	//!Mil001 Note: this value is not used.
...
var HEBREW_EPOCH = 347995.5;	//!Mil001 Attention, this value is for the second day before first day at midnight.
...
{
    var months, day, parts; //!Mil003 day instead of days
...
function jd_to_hebrew(jd)
{
    ...
    jd = Math.floor(jd - 0.5) + 0.5; //!Mil001 Replaced Math.floor(jd) with Math.floor(jd - 0.5)
...
//!Mil003 var ISLAMIC_WEEKDAYS transferred
...
//!Mil003 var PERSIAN_WEEKDAYS transferred
/*
...
//!Mil003 var MAYAN_HAAB_MONTHS transferred
...
//!Mil003 var MAYAN_TZOLKIN_MONTHS transferred
...
//!Mil003 var INDIAN_CIVIL_WEEKDAYS transferred
...
function updateFromGregorian()
{
    var j, year, mon, mday, hour, min, sec,
        weekday, julcal, hebcal, islcal, perscal, hmindex, utime, isoweek, //!Mil003 added perscal
    ...
    switch (hebrew_year_days(hebcal[0])) {
        case 353:
            document.hebrew.leap.value = HEBREW_YEAR_TYPES[0]; 
            break;

        case 354:
            document.hebrew.leap.value = HEBREW_YEAR_TYPES[1];
            break;

        case 355:
            document.hebrew.leap.value = HEBREW_YEAR_TYPES[2];
            break;

        case 383:
            document.hebrew.leap.value = HEBREW_YEAR_TYPES[3];
            break;

        case 384:
            document.hebrew.leap.value = HEBREW_YEAR_TYPES[4];
            break;

        case 385:
            document.hebrew.leap.value = HEBREW_YEAR_TYPES[5];
            break;

        default:
            document.hebrew.leap.value = HEBREW_YEAR_TYPES[6] +
                hebrew_year_days(hebcal[0]) + HEBREW_YEAR_TYPES[7];
            break;
    }
    ...
    document.islamic.wday.value = ARAB_DAY + ISLAMIC_WEEKDAYS[weekday]; //!Mil003 ARAB_DAY used
    ...
    //  Update Excel 1900 and 1904 day serial numbers
		...
		document.excelserial1900.day.value = (j - J1900MS) + 1;	//!Mil001 this line replaces the following ones.
//!    document.excelserial1900.day.value = (j - J1900) + 1 +
            /*  Microsoft marching morons thought 1900 was a leap year.
                ...
                idiocy.  */
//! With the newest version of Excel (After 2016-2017 on Windows and McOS)
//! this bug may bypassed in this way:
//! enter a date between 1900-01-01 and 1900-02-28 as a character string
//! and then use date functions directly on string.
//! This trick can be used on any date downto 100-01-01.
//! Hence it should be considered that Excel epoch (day 1) is 1899-12-31
//! and not make any particular case of the period between 1900-01-01 ane 1900-02-28
//!				((j > 2415078.5) ? 1 : 0) //!Mil001 not used anymore
        ;
...
{
    updateFromGregorian();
//!Mil001 Doing so, the gregorian fields are not updated. Let us add a second pass
	calcJulian();	//!Mil001 Update gregorian fields
}
...
function calcJulian()
{
    ...
    document.gregorian.hour.value = time[0]; //pad(time[0], 2, " ");
    document.gregorian.min.value = time[1]; //pad(time[1], 2, "0");
    document.gregorian.sec.value = time[2]; //pad(time[2], 2, "0");
    updateFromGregorian();
}
...
function calcExcelSerial1900()
{
	...
	/*With Office 365, it is noticeable that Excel 1900 origin
	corresponds to 1899-Dec-30
	even if any date expression from 1900-01-01 to 1900-02-29
	shall be "translated" to the date of the next day.
	*/
...
function presetDataToRequest(s)
{
	...
	if (calendar.toLowerCase() == CALENDAR_NAMES.gregorian) {	//!Mil003 use CALENDAR_NAMES
	    ...
	    if (d != null) {
		    ...
		    set = true;
	    	    ...
	    	    alert(ALERT_MSG.Invalid_Greg_date + date +		//!Mil003 use ALERT_MSG
				ALERT_MSG.In_search_request);
		}
	    } else {
			alert(ALERT_MSG.Invalid_Greg_date + date +		//!Mil003 use ALERT_MSG
			ALERT_MSG.In_search_request);
	...
	} else if (calendar.toLowerCase() == CALENDAR_NAMES.julian) {	//!Mil003 use CALENDAR_NAMES
	    var d = date.match(/^(\d+)\D(\d+)\D(\d+)(\D\d+)?(\D\d+)?(\D\d+)?/);
	    if (d != null) {
	    	// Sanity check date and time components
	    	if ((d[2] >= 1) && (d[2] <= 12) &&
		    ...
		    set = true;
				...
				alert(ALERT_MSG.Invalid_Jul_date + date +		//!Mil003 use ALERT_MSG
				ALERT_MSG.In_search_request);
		}
	    } else {
	    	alert(ALERT_MSG.Invalid_Jul_date + date +		//!Mil003 use ALERT_MSG
	...
	} else if (calendar.toLowerCase() == CALENDAR_NAMES.jd) {	//!Mil003 use CALENDAR_NAMES
	    var d = date.match(/^(\-?\d+\.?\d*)/);
	    if (d != null) {
	    	setJulian(d[1]);
		set = 1;
	    } else {
	    	alert(ALERT_MSG.Invalid_JD + date +		//!Mil003 use ALERT_MSG
		    ALERT_MSG.In_search_request);
	    }
	    
	} else if (calendar.toLowerCase() == CALENDAR_NAMES.mjd) {	//!Mil003 use CALENDAR_NAMES
	    var d = date.match(/^(\-?\d+\.?\d*)/);
	    if (d != null) {
		...
		set = 1;
	    	...
	    	alert(ALERT_MSG.Invalid_MJD + date +		//!Mil003 use ALERT_MSG
		    ALERT_MSG.In_search_request);
	    }
	    
	} else if (calendar.toLowerCase() == CALENDAR_NAMES.unixtime) {	//!Mil003 use CALENDAR_NAMES
	    var d = date.match(/^(\-?\d+\.?\d*)/);
	    if (d != null) {
		...
		set = 1;
	    	...
	    	alert(ALERT_MSG.Invalid_Unix + date +		//!Mil003 use ALERT_MSG
			ALERT_MSG.In_search_request);
	    }
	    
	} else if (calendar.toLowerCase() == CALENDAR_NAMES.iso) {	//!Mil003 use CALENDAR_NAMES
	    var d;
		...
		set = 1;
	    	...
	    	alert(ALERT_MSG.Invalid_ISO  + date +		//!Mil003 use ALERT_MSG
			ALERT_MSG.In_search_request);
	    }
	    
	} else if (calendar.toLowerCase() == CALENDAR_NAMES.excel) {	//!Mil003 use CALENDAR_NAMES
	    var d = date.match(/^(\-?\d+\.?\d*)/);
	    if (d != null) {
		...
		set = 1;
	    	...
	    	alert(ALERT_MSG.Invalid_EXL + date +		//!Mil003 use ALERT_MSG
			ALERT_MSG.In_search_request);
	    }
	    
	} else if (calendar.toLowerCase() == CALENDAR_NAMES.excel1904) {	//!Mil003 use CALENDAR_NAMES
	    var d = date.match(/^(\-?\d+\.?\d*)/);
	    if (d != null) {
		...
		set = 1;
	    	...
	    	alert(ALERT_MSG.Invalid_EXL04 + date +		//!Mil003 use ALERT_MSG
			ALERT_MSG.In_search_request);
	    }
	
	} else {
	    alert(ALERT_MSG.Invalid_calendar + calendar +		//!Mil003 use ALERT_MSG
			ALERT_MSG.In_search_request);
	}
    } else {
    	alert(ALERT_MSG.Invalid_search_request + s);		//!Mil003 use ALERT_MSG
    }
